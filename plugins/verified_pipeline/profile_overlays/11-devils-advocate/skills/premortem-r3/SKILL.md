...
7. Calculate the aggregate `score` only from `da-request.json.risk_policy`: start at `score_base`, subtract the configured classification penalty once for every unresolved finding, subtract nothing for resolved findings, and clamp the result at `score_floor`. Therefore zero findings scores exactly `score_base` (normally 100); it is not a residual-risk score.
