...
7. Calculate the aggregate `score` only from `da-request.json.risk_policy`. For `da-request/v2`, start at `score_base` and clamp at `score_floor`; for legacy `da-request/v1`, use base 100 and floor 0. In both versions, subtract the configured classification penalty once for every unresolved finding and subtract nothing for resolved findings. Therefore zero findings score exactly 100; this is not a residual-risk score.
