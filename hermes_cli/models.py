...
def opencode_provider_family(provider_id: Optional[str]) -> Optional[str]:
    ...
    Returns ``"opencode-zen"``, ``"opencode-go"``, or ``"opencode-free"`` for the built-in
    ...
    Family slugs match exactly or as a hyphen-delimited custom-provider prefix;
    lookalikes such as ``opencode-goober`` must not be classified as OpenCode.
    """
    ...
    if canonical in {"opencode-zen", "opencode-go", "opencode-free"}:
        return canonical
    if raw == "opencode-free" or raw.startswith("opencode-free-"):
